...
// QueryMapRecords runs a query in the db and converts them map[string]interface{}
func (dbu *PGSqlDBUtil) QueryMapRecords(sql string, params ...interface{}) ([]interface{}, []map[string]interface{}, error) {
	dbu.refreshConnection()
	rslts, err := dbu.dbConnection.Query(context.Background(), sql, params...)
	if err != nil {
		_dbUtillog.Errorf("Error in executing the query %v", err)
	...
	defer rslts.Close()
	rows := make([]interface{}, 0)
	rowsInMap := make([]map[string]interface{}, 0)
	var colMap map[int]string
	for rslts.Next() {
		objMap := make(map[string]interface{})
		...
		rows = append(rows, objMap)
		rowsInMap = append(rowsInMap, objMap)
	}

	return rows, rowsInMap, err
}
