...
var (
	_typeCache    map[string]*SQLFieldMap
	_sqlHelperLog *logrus.Logger
)

var (
	_strPtrType   = reflect.PtrTo(reflect.TypeOf(string("")))
	_intPtrType   = reflect.PtrTo(reflect.TypeOf(int(0)))
	_floatPtrType = reflect.PtrTo(reflect.TypeOf(float64(0.0)))
	_dtPtrType    = reflect.PtrTo(reflect.TypeOf(time.Now()))
	_intType      = reflect.TypeOf(int(0))
	_floatType    = reflect.TypeOf(float64(0))
	_strType      = reflect.TypeOf(string(""))
	_dtType       = reflect.TypeOf(time.Now())
)
	...
	for key, value := range genericObj {
		if key != "_id" {
			if dataType, isExisting := sfm.FieldMap[key]; isExisting {
				...
				valueBuffer.WriteString(",")
				pos++
				_dbUtillog.Infof("For Insert :  Data type %s->%v", key, reflect.TypeOf(value))
...
func (sfm *SQLFieldMap) GenerateUpdateScript(obj interface{}, filter map[string]interface{}) (string, []interface{}) {
	...
	isFirst := true
	for key, value := range genericObj {
		_dbUtillog.Infof("For Update : Data type %s->%v", key, value)
